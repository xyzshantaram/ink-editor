...
var download = (filename, text) => {
  ...
  elt2.setAttribute("href", "data:text/plain;charset=utf-8," + encodeURIComponent(text));
  elt2.setAttribute("download", filename);
...
var createCmEditor = (ph) => {
  return new EditorView({
    extensions: editorExtensions(ph),
...
var setup = (defaultContent, prompts, placeholder2) => {
  const editor = createCmEditor(placeholder2);
  ...
  const handlers2 = {
    ...
    "h2": () => insert2.before("## ", 3),
    "done": () => {
      download(`writr-bio-${(/* @__PURE__ */ new Date()).toJSON()}.md`, editor.state.doc.toString());
    },
    "prompt": showPrompts,
...
var init = (root, defaultContent = "", prompts = [], placeholder2 = "") => {
  let rootDiv;
  ...
  const { editor } = setup(defaultContent, prompts, placeholder2);
  const getVal = () => {
